...
* Необходимо также, в свойстве `'modules'` разкоментировать `'gii'` - для использования генератора кода.
...
* Для использования баз данных *mysql* нужно разкоментировать соответствующее свойство `'db'` для настроек *mysql* и закоментировать `'db'` для *sqlite*.  
