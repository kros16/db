...
* В свойстве `'components'` раскомментировать `'urlManager'` - для использования ЧПУ. Добавим в него свойство `'showScriptName'=>false,` - для того, что бы убрать название выполняемого скрипта из URL.

* Для использования баз данных *mysql* нужно раскомментировать соответствующее свойство `'db'` для настроек *mysql* и закомментировать `'db'` для *sqlite*.  
